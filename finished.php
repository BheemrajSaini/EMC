...
<?php
		...
		if(1)
		{
			$sql="Select id,name,designation,department,location,description,processed,area,time,room, dispatchedTime,finishedTime,contactPerson, contactNumber, timing,descText,availablefrom,availableto,contact from complaints ";
		...
		$sql="Select id,name,designation,department,location,description,processed,area,time, contactPerson, contactNumber, room,timing,descText,availablefrom,availableto,contact, dispatchedTime,finishedTime from complaints where processed=2 order by time ASC";
...
function rowover(id,str,time,contact,descText)
{
	...
	document.getElementById("descdiv").innerHTML+="<br><h4>Availablity Time : </h4>" + myTime(document.getElementById("tm"+str).value)
	if(contact!='')
