...
<?php 
		...
		{
			$flag = 0;
			$_POST['selCom']=clean($_POST['selCom']);
			//echo $_POST['selCom'];
			/*removed*/
			for($i=1;$i<=$_POST['cntItems'];$i++)			
			{
			       //echo $_POST['cntItems'];
                            // echo $_POST["finalItem".$i];
...
//exit();
			...
			/*end*/
				...
				for($i=1;$i<=$_POST['cntItems'];$i++)							
				{
					...
					$sql="Update complaints set indent_status=1 where id= '".$_POST['selCom']."' ";
...
if(f.selCom.value=="0")
	{	alert("Please Select ComplaintNo.")
		f.selCom.focus()
		x = 0;
		blinkBorder("white","red", f.selCom, 500);
		return false;
	}
...
<select id ="selCom" name="selCom">
<option name="opt0"value="0">Select</option>
<?php
$i=1;
	$result=mysql_query("Select id from complaints where processed=1 and indent_status=0",$conn);
	while($row=mysql_fetch_array($result))
	{	if ($row[0]!="") {
?>
<option name="opt".$i value="<?php echo $row[0] ?>"><?php echo "EL".str_pad($row[0],5,"0",STR_PAD_LEFT);?></option>
$i++;
<?php
	}}
?>
...
<!--<center><input align="center" type="submit" name="Submit" "value="Submit" /></center>-->
</center><input type="submit" name="Submit" value="Submit"  onclick="return confirm('Are you sure you want to submit?');" /></center>
</div>
