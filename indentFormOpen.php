			...
			if($result and $num>0)
			{
				...
				if(mysql_query("Update materials set status=2, indentClose_date='". $time."' where status<2 and indentNo=".$arr[$i]))
				{
					...
					for($j=1;$j<=$row['count'];$j++)	
					{
						...
						$items.=$str."<br />";					
						$sql = "INSERT INTO report VALUES ('', '" .  $_POST['itemName'.$j] . "', " . ($_POST['existing'.$j] - $_POST['returned'.$j]) . ", '" .$time ."', '". $_POST['finalCloseID']. "')";					
						mysql_query($sql);	
//						$sql = "INSERT INTO report VALUES ('', '" .  $_POST['itemName'.$j] . "', " . $_POST['returned'.$j] . ", '" .$time ."', '". $_POST['finalCloseID']. "')";
					}
				}			
			}		
...
function callFrame()
{
	TTTID = document.getElementById('searchText').value;
	TTTID = parseInt(TTTID);	
	document.getElementById('fetchFrame').src = "getRow.php?fetchID="+TTTID;
	document.getElementById('fetchFrame').style.visibility='visible';	
	//alert(document.getElementById('fetchFrame').src);
}
/*end*/
...
<div id="menu" >
 ...
 <!--start-->
 <iframe frameborder="0" id="fetchFrame" style="visibility:hidden; margin-right:100px" height = "500px" width="1000px"></iframe>
 <br /><br />
 <!--end-->
...
<!--start-->
<input style="position:absolute;top:170px;right:105px;" type="button" onclick = "document.getElementById('mainContents').style.visibility='hidden'; callFrame();" id = "searchBtn" name="nm" value="search" />
<!--end-->

</form>
<!--start-->
<div id="mainContents">
<form style="position:absolute;top:200px;left:5px;" typid="form1" name="form1" method="post">
<!--end-->
<div id="content">
<div id="ASright">
...
<div align ="right" >
<div id = "closeIndent" align="right" style="position:relative;top:50px;right:60px;">
</div>
</div>
</div>
<input type="button" onclick = "checkSubmit();" id="saveSubmit" name="btnSave"  value="Save" style="display:none; margin-left:850px; margin-top:60px;"/>
<input type="submit" id="closeSubmit" name="btnClose"  value="Close" style="display:none; margin-left:850px; margin-top:60px; display:none;"/>
