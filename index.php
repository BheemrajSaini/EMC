...
<?php 
	if(isset($_POST['Submit']))
	{
		...
		if(!isset($_POST['t4'])) $_POST['t4']=0;
		if(!isset($_POST['t5'])) $_POST['t5']=0;
		if(!isset($_POST['t6'])) $_POST['t6']=0;
		if(!isset($_POST['t7'])) $_POST['t7']=0;
		$sql.="'".$_POST['t4']."',";
		$sql.="'".$_POST['t1']." - " . $_POST['t2']." - " . $_POST['t3']." - " . $_POST['t5'] ." - ".$_POST['t6']." - ".$_POST['t7']."'";			
...
<script>
	...
	{
		var f=document.form1
		var tr1=0,tr2=0,tr3=0,tr4=0, tr5=0,tr6=0,tr7=0
		if(f.t1.checked)tr1=1
		if(f.t2.checked)tr2=2
		if(f.t3.checked)tr3=4
		if(f.t4.checked)tr4=8
		if(f.t5.checked)tr5=16
		if(f.t6.checked)tr6=32
		if(f.t7.checked)tr7=64
	...
	function validate()
	{
		...
		{	alert("Please Enter Quarter Number. (Only Numbers, max 3 digits)")
			...
			*/
		if(!f.t1.checked && !f.t2.checked && !f.t3.checked && !f.t4.checked && !f.t5.checked && !f.t6.checked && !f.t7.checked)
		{
		...
		if(!f.p1.checked && !f.p2.checked && !f.p3.checked && !f.p4.checked && !f.p5.checked && !f.p6.checked && !f.p7.checked && !f.p8.checked && !f.p9.checked)
		{
		...
		if(f.descBox.value=="" && f.p9.checked)
		{
	...
	{
		var f=document.form1
		if(f.designation.value=="Faculty" || f.designation.value=="Staff")
		{
			document.getElementById("Faculty").style.display='block'
			document.getElementById("nonFaculty").style.display='none'
		}
		else
		{
			document.getElementById("Faculty").style.display='none'
			document.getElementById("nonFaculty").style.display='block'
		}
/*
			document.getElementById(i++).style.display='none'
		if(document.form1.comlocation.value)
	...
	<div id="content">
		<div id="right">
			...
			<table width="95%" align="center" border="0" cellpadding="5" cellspacing="5">
                            <tr><td align ="right"><b><font color="red" size="1"> * All Feilds Are Mandatory </font></b></td><td></td><td></td><td></td></tr>
				<tr align="center">
					...
					<td align="left"><input type="checkbox" name="p1" value="1" />Fan is not working.<br /></td>
				</tr>
				<tr align="center">
					...
					<td></td>
					<td align="left"><input type="checkbox" name="p2" value="2" />Tubelight is not working.</td>
				</tr>
				...
				<tr align="center">
					...
					<td></td>
					<td align="left"><input type="checkbox" name="p3" value="4" />Switch not working.</td>
				</tr>
				<tr align="center">
					...
					<td align="left"><select name="comlocation" onchange="show();">
						...
						<option value="5">Other</option>
							...
							<td>&nbsp;</td>
					<td align="left"><input type="checkbox" name="p4" value="8" />Plug Point is not working.</td>
...
?>
							...
							&nbsp;&nbsp;<br>&nbsp;<br>Quarter No. :<input type="text" name="quarterno" value="" />
						</div>
...
?>
						...
						<div style="display:none" id="5">
							<input type="text" name="comlocation25" id="comlocation25" size="26" />
<?php /* <!--<select name="comlocation25" id="comlocation25">-->
<?php 
...
?>
							...
							<input type="checkbox" name="o3" value="4"  />Other Place<br/>
						</div>
					</td>
					<td align="left"><input type="checkbox" name="p5" value="16" />Street Light is not working.</td>
					<td>&nbsp;</td>
				</tr>
				<tr>
					...
					<td align="left">
						...
						<div style="display:none" id="Faculty">
							...
							<input type="checkbox" name="t6" value="morning (10am to 12:30pm)" />Morning (10am to 12:30pm)<br />
							<input type="checkbox" name="t7" value="Evening(3pm to 5:30pm)"  />Evening(1:30pm to 5:00pm)<br />
					
						</div>
					
						<div style="display:block" id="nonFaculty">
							...
							<input type="checkbox" name="t5" value="Evening(5:30pm to 7pm)" />Evening(5:30pm to 7pm)<br />
					...
					<td>&nbsp;</td>
					<td align="left">
						<input type="checkbox" name="p6" value="32" />Power Failure (Fuse)<br /><br />
						<input type="checkbox" id = "ac" name="p7" value="64" />Air Conditioner<br /><br />
						<input type="checkbox" id = "lift" name="p8" value="128" />Lift<br />
						</td>
					...
					<td align="left"><input type="text" name="contact" maxlength="15" /> </td>
						...
						<td align="left">Problem Description:</td>
						<td align="left">
						
						<input type="checkbox" id = "descCheck" name="p9" value="256" />Others <br />
	...
	<div class="box">
					<b>Use Your Webmail ID as USERNAME & Webmail Password as PASSWORD</b>
